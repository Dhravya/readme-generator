...
fn main() {
    ...
    stdin().read_line(&mut project_name).expect("Failed to read line");

    let mut short_description = String::with_capacity(120);
    println!("{}","Enter the short description (It should be short, concise to hook the reader) : ".blue());
    stdin().read_line(&mut short_description).expect("Failed to read line");

    let mut image_url = String::new();
    println!("{}","Enter the image url (Leave blank if none): ".blue());
    stdin().read_line(&mut image_url).expect("Failed to read line");

    let mut license = String::new();
    println!("{}","Enter the license - (MIT/Apache/GPL / Enter any custom license) (Leave blank if none): ".blue());  
    stdin().read_line(&mut license).expect("Failed to read line");

    let mut demo = String::new();
    println!("{}","Link for image/gif demonstrating your project ".blue());
    ...
    stdin().read_line(&mut installation_command).expect("Failed to read line");

    // Checks
    assert!(project_name.trim().len() > 0, "Project name cannot be empty");
    assert!(short_description.trim().len() > 0, "Short description cannot be empty");

    file_factory(project_name, short_description, image_url, license, demo, installation_command);
}


fn file_factory(project_name:String, 
    ...
    content.push_str(
        licenses.get(license.trim().to_lowercase().as_str())
            .unwrap_or(&format!("<img alt=\"License: {}\" src=\"https://img.shields.io/badge/License-{}-blue\" />", license.trim(), license.trim()).as_str())
    );
    content.push_str("<br>\n");
    ...
    if image_url.len() > 5 {
        content.push_str(format!("<img src=\"{}\" alt=\"{}\" width=\"500\" height=\"500\">\n", image_url.trim(), project_name.trim()).as_str());
    }
